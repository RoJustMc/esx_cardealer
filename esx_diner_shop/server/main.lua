...
MySQL.ready(function()
	MySQL.Async.fetchAll('SELECT * FROM diner_shop LEFT JOIN items ON items.name = diner_shop.item', {}, function(shopResult)
		for i=1, #shopResult, 1 do
			if shopResult[i].name then
				...
				print(('esx_diner_shop: invalid item "%s" found!'):format(shopResult[i].item))
			end
		end
	end)
end)

ESX.RegisterServerCallback('esx_diner_shop:requestDBItems', function(source, cb)
	cb(ShopItems)
end)

RegisterServerEvent('esx_diner_shop:buyItem')
AddEventHandler('esx_diner_shop:buyItem', function(itemName, amount, zone)
	...
	if amount < 0 then
		print('esx_diner_shop: ' .. xPlayer.identifier .. ' attempted to exploit the shop!')
		return
	end
